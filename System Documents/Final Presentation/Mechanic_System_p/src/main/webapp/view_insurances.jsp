          ...
          <div class="row">
                ...
                <div class="card-body" id="new_booking">
                  <div class="table-responsive">
                     <table class="table" id="table_bookings">
                       ...
                       <tbody>
                         <tr>
                           <td>${ insurance.reg_number }</td>
                           <td>${ insurance.company_name }</td>
                           <td>${ insurance.phone }</td>
                           <td>${ insurance.email }</td>
                           <td>${ insurance.address }, ${ insurance.city }, ${ insurance.country }, ${ insurance.zip_code }</td>
                           <td>
                           <c:if test="${mechanic_user.roles == 'ADMIN, MECHANIC'}">
                           		<form action="/delete_insurance">
								<div class="dropdown">
									<button class="btn btn-primary icon-btn dropdown-toggle" type="button" id="dropdownMenuIconButton6" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
										<i class="fas fa-2x text-gray-300"> Actions </i>
									</button>
									<div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton6">
										<input type="hidden" name="reg_number" id="booking_id" value="${ insurance.reg_number }">
										<button class="dropdown-item" type="submit"> Delete Insurance </button>
										<c:choose>
											<c:when test="${ insurance.active == 1 }">
												<button class="dropdown-item" type="submit" formaction="/mute_insurance"> Mute Insurance </button>
											</c:when>
											<c:when test="${ insurance.active == 0 }">
												<button class="dropdown-item" type="submit" formaction="/mute_insurance"> Make Active </button>
											</c:when>
										</c:choose>
									</div>
								</div>
            ...
            <div class="col-md-4 grid-margin">
              <div class="card shadow mb-4">
                <!-- Card Header - Dropdown -->
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary"> Insured Cars </h6>
                </div>
                <!-- Card Body -->
                <c:forEach items="${ insurance_list }" var = "insurance">
                <div class="card-body" id="ins_${ insurance.reg_number }">
                    <div class="table-responsive">
                      <table class="table table-stretched">
                        <thead class="">
                          <tr>
                            <th><b> Car </b></th>
                            <th><b> Service </b></th>
                            <th><b> Car Status </b></th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr>
                            <td>
                              <p class="mb-1 text-dark font-weight-medium"> ${ insurance.cars[0].booking.brand } </p>
                              <small class="font-weight-medium"> ${ insurance.cars[0].booking.model }, ${ insurance.cars[0].booking.car_year } </small>
                            </td>
                            <td class="font-weight-medium"> ${ insurance.cars[0].booking.service_type } </td>
                            <td class="text-success font-weight-medium"> ${ insurance.cars[0].carStatus } </td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
